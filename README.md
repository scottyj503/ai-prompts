...
Follow-up pattern: after the author pushes, `/review1 --update` (or its alias `/codeReviewUpdate <sha>` in the same session) runs review1's Follow-up section — it diffs from the reviewed SHA, re-invokes the workflow on only the changed files with the prior actionable findings as `priorFindings`, shows each as RESOLVED, OPEN, or WITHDRAWN with evidence, numbers new findings by continuing the original sequence, refreshes the `--out` report, and offers to post only the new findings. It also reads the author's replies on each posted comment thread since the review and hands them to the Recheck agent as untrusted claims: a "fixed" claim the code does not bear out stays OPEN with counter-evidence, a rebuttal the code confirms closes the finding as WITHDRAWN, and threaded replies (acknowledgement, counter-evidence, or an answer to a question) are offered for posting into the existing thread. Posted comment bodies carry the finding ID so replies map back reliably. `--update` re-injects the full command instructions, so it stays reliable after a long session's context has been summarized; with a report path it also works in a fresh session.
...
- **v7.0 (2026-09)**: Workflow-orchestrated PR review, ported from [srhoton/dotfiles PR #53](https://github.com/srhoton/dotfiles/pull/53)
  ...
  - Added `workflows/review1-fanout.js` — the orchestration script. Skeptics inherit the session model by default (`skepticModel` / `skepticEffort` args override) so models can be compared across runs. Follow-up mode via `priorFindings` adds a Recheck phase (RESOLVED/OPEN/WITHDRAWN per prior finding, plus a classification of the author's reply) so `/review1 --update` reconciles against the original numbering and the author's rebuttals instead of re-reading free-form. No per-agent verdict; review action defaults to Comment so developers can refute, with a nudge when a verified CRITICAL exists. CRITICAL/HIGH/MEDIUM pre-selected for posting.
