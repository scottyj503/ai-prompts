...
#### Issues Found
1. **[CRITICAL]** [Description] — `file:line` — [Suggested fix]
2. **[HIGH]** [Description] — `file:line` — [Suggested fix]
...
- **CRITICAL** — Prevents approval. Wrong behavior, missing core functionality, failing tests for reviewed component.
- **HIGH** — Should be fixed but doesn't prevent basic functionality from working.
- **MEDIUM** — Minor functional concern, worth noting but not blocking.
- **LOW** — Noted for awareness, does not affect verdict.

Only CRITICAL issues result in a NEEDS_CHANGES verdict.
...
When dispatched with explicit output format instructions (e.g., from a pipeline or routing agent), follow the dispatch format instead of the default template in the "Output Format" section above. The dispatch instructions take precedence for structure and field names — your analysis approach and thoroughness remain the same.
